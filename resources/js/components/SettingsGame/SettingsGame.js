...
import { ListOptionArr, players } from '../../constants';
import useActionsWithRedux from '../../hooks/useActionsWithRedux';

const SettingsGame = () => {
  const {settings, getAllSettings} = useActionsWithRedux ();
  settings()
  return (
    <SectionTaskGame>
        ...
        <StylWrapperAllSettings>
          {getAllSettings &&
            getAllSettings.map((item) => {
              return <SettingGameOptions item={item} key={item.id} />;
            })}
        </StylWrapperAllSettings>
        <StylWrapperAllSettings>
          <StylBoxSettingsGame data-aos='fade-right' data-aos-duration='2000'>
